...
from ansys.solutions.my_osl_solution.model.optislang.project_tree import dump_project_state, get_project_tree, get_node_list, get_step_list, get_node_hids
from ansys.solutions.my_osl_solution.model.optislang.install import get_available_optislang_installations
from ansys.solutions.my_osl_solution.ui.utils.monitoring import read_log_file
...
class ProblemSetupStep(StepModel):
    ...
    # Inputs
    project_file: FileReference = FileReference("Problem_Setup/arch.opf")
    properties_file: FileReference = FileReference("Problem_Setup/arch.json")
    ...
    def generate_project_state(self) -> None:
        ...
        project_file = Path(__file__).absolute().parent.parent / "model" / "assets" / "arch.opf"
    ...
    def upload_bulk_files_to_project_directory(self) -> None:
        ...
        original_project_file = Path(__file__).parent.absolute().parent / "model" / "assets" / "arch.opf"
        self.project_file.write_bytes(original_project_file.read_bytes())

        original_properties_file = (
            Path(__file__).parent.absolute().parent / "model" / "assets" / "arch.json"
        )
