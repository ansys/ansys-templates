...
def display_page_body(pathname, n_clicks):
...
def display_page_content(pathname, value, n_clicks):
    """
    ...
    if triggered_id == "navigation_tree":
        if value is None:
            ...
            if project.steps.problem_setup_step.project_started:
                node_info, index = extract_dict_by_key(project.steps.problem_setup_step.step_list, "key", value, expect_unique=True, return_index=True)
                project.steps.problem_setup_step.selected_actor_info = node_info
                project.steps.problem_setup_step.selected_actor_index = index
                page_layout = monitoring_page.layout(project.steps.problem_setup_step)
            else:
                page_layout = problem_setup_page.layout(project.steps.problem_setup_step)
        return page_layout
